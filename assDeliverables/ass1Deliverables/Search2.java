 ...
 */
class SearchTask2 implements Callable<List<Integer>> {
     ...
     * text[from..(to-1)]
     */
    public SearchTask2(char[] text, char[] pattern, int from, int to) {
        this.text = text;
...
public class Search2 {
    ...
    public static void main(String[] argv) {
        try {
             ...
             *********************************************/
            SearchTask2 singleSearch = new SearchTask2(text, pattern, 0, len);
            ...
            List<SearchTask2> taskList = new ArrayList<>();
            ...
            if (ntasks > upperTaskLimit)
                throw new IllegalStateException("The number of tasks exceeds the upper limit of tasks possible for the request. The upper limit of tasks is: " + upperTaskLimit + " (len of file - (pattern.len - 1))");

            int partition = (int)Math.ceil((len - (pattern.length - 1)) / (double)ntasks);
            int u;
            for (u = 0; u < ntasks - 1; ++u)
                taskList.add(new SearchTask2(text, pattern, u * partition, (u + 1) * partition + pattern.length - 1));
            taskList.add(new SearchTask2(text, pattern, u * partition, len));
